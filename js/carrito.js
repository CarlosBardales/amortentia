    ...
    if (producto) {
      ...
      itemContainer.innerHTML = `
                    ...
                    <div>
                        ...
                        <p><strong>Mensaje para la tarjeta:</strong> ${
                          item.mensajeTarjeta || "Ninguno"
                        }</p>
                        <p><strong>Turno de entrega:</strong> ${
                          item.turnoEntrega || "No especificado"
                        }</p>
                        <p><strong>Fecha de entrega:</strong> ${
                          item.fechaEntrega || "No especificado"
                        }</p>
                        <div class="acciones">
                            <button class="agregar-mas" data-id="${
                              producto.id
                            }">Agregar más</button>
...
function generarEnlaceWhatsApp() {
  ...
  productosUnicos.forEach((item) => {
    ...
    if (producto) {
      total += producto.precio * item.cantidad;
      mensaje += `${producto.nombre} x ${item.cantidad} - S/${
        producto.precio * item.cantidad
      }\n`;

      // Agregar los detalles adicionales del producto al mensaje
      mensaje += `Mensaje para la tarjeta: ${
        item.mensajeTarjeta || "Ninguno"
      }\n`;
      mensaje += `Turno de entrega: ${
        item.turnoEntrega || "No especificado"
      }\n`;
